...
{
    global $conn;
    if (mysqli_num_rows(mysqli_query($conn, "SELECT id FROM users WHERE username='$username';")) == 0) {
        return false;
    } else {
        return true;
    }
...
{
    global $conn;
    return mysqli_fetch_assoc(mysqli_query($conn, "SELECT id FROM users WHERE username='$username';"))['id'];
}
